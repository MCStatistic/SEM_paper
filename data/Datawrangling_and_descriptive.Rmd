...
##Einstellung zu Ausländern - Variablen mit "negativen Aussagen"
mp02 - Soziales Netz
mp04 - Wohnungen
mp06 - Arbeitsplätze
mp07 - Straftaten
...
mc09 (Wie oft gute Erfahrungen mit Kontakt gemacht?) -> KEIN Multigruppenvergleich, da zu viel...
mc01, mc02, mc03 -> entfällt, da Faktorladungen zu schlecht (entsprechende Modellierung in den Anhang)
...
glm.fit1 <- glm(siops08_mis ~ work + age + sex, 
                data = allbus_oD, 
                family = binomial)

glm.fit2 <- glm(siops08_mis ~ work + age + sex + di08c, 
                data = allbus_oD, 
                family = binomial)
...
# deleting the row with one new NA in it.
...
# logistische Regression - mc04 missing (Kontakt)
allbus_c <- transform(allbus_c, mc04_mis = ifelse(is.na(mc04), 0, 1))
glm_mis1 <- glm(mc04_mis ~ sex + age, 
                data = allbus_c, 
                family = binomial)
summary(glm_mis1)

# 
glm_mis2 <- glm(mc04_mis ~ sex + age + isced11 + id02, 
                data = allbus_c, 
                family = binomial)
summary(glm_mis2)
...
glm_mis3 <- glm(di08c_mis ~ sex + age + isced11, 
                data = allbus_c, 
                family = binomial)
summary(glm_mis3)
stargazer(glm_mis1, glm_mis2, glm_mis3, title = "Tabelle x: logisitische Regression - Fehlende Werte")
...
# combining the original dataset with the first imputated dataset
...
results # Mardia Skewness ist extrem hoch, leider derzeit noch keine wirkliche Literatur diesbezüglich gefunden
