...
allbus <- read_spss("allbus2016.sav") %>% 
  select(age, ma09, mp01, mp02, mp03, mp04, mp05, mp06, mp07, mp08, mp09, mp10, mp11, mp12, 
         ...
         mn15, mn16, mn17, mn18, mn19, mn20, mn21, xr27)
...
ggplot(data = missing, aes(variable, pct_miss)) + 
  geom_col(position = position_stack(reverse = TRUE))

# Plot of Percentage of Missing Values for each Variable
ggplot(data=missing, aes(x= reorder(variable,pct_miss), 
                         y=pct_miss,
                         label=pct_miss)) +
  labs(x = "Variables", y = "Percentage of Missing Values") +
  geom_bar(stat="identity", fill = "skyblue4") + 
  geom_text(label=format(missing$pct_miss, digits=2),
            size = 3,
